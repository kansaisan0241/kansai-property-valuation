...
function DetailIcon({ label }: { label: string }) {
  ...
  if (label === "所在地") return <svg viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M12 21s6-5.4 6-11a6 6 0 1 0-12 0c0 5.6 6 11 6 11Z" /><circle {...common} cx="12" cy="10" r="2.2" /></svg>;
  if (label.includes("面積")) return <svg viewBox="0 0 24 24" aria-hidden="true"><rect {...common} x="4" y="4" width="16" height="16" /><path {...common} d="M9.3 4v16M14.7 4v16M4 9.3h16M4 14.7h16" /></svg>;
  if (label === "築年月") return <svg viewBox="0 0 24 24" aria-hidden="true"><rect {...common} x="3.5" y="5.5" width="17" height="15" rx="1.5" /><path {...common} d="M7 3.5v4M17 3.5v4M3.5 9.5h17M7 13h2M11 13h2M15 13h2M7 17h2M11 17h2" /></svg>;
  if (label.includes("交通")) return <svg viewBox="0 0 24 24" aria-hidden="true"><circle {...common} cx="12" cy="12" r="7.5" /><circle {...common} cx="12" cy="12" r="3" /><path {...common} d="M12 4.5V2.8M12 21.2v-1.7M4.5 12H2.8M21.2 12h-1.7" /></svg>;
  if (label.includes("階")) return <svg viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M5 21V4h14v17M8.5 8h2M13.5 8h2M8.5 12h2M13.5 12h2M8.5 16h2M13.5 16h2M3 21h18" /></svg>;
  if (label === "接道状況") return <svg viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M8 2 5 22M16 2l3 20M12 4v3M12 10v4M12 17v3" /></svg>;
  if (label === "間取り") return <svg viewBox="0 0 24 24" aria-hidden="true"><rect {...common} x="3" y="3" width="18" height="18" /><path {...common} d="M10 3v8H3M10 8h6V3M16 8v13M10 15h6M7 11v10" /></svg>;
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M3 11.5 12 4l9 7.5M5.5 10v10h13V10M9 20v-6h6v6" /></svg>;
}

function DetailEditor({ icon: _icon, label, value, onChange, type = "text", secondary, options, className = "", printVisible = true, onTogglePrint }: { icon: string; label: string; value: string | number; onChange: (value: string) => void; type?: string; secondary?: string; options?: { value: string; label: string }[]; className?: string; printVisible?: boolean; onTogglePrint?: () => void }) {
  return <label className={`detail-item ${type === "textarea" ? "multiline-detail" : ""} ${!printVisible ? "print-optional-hidden" : ""} ${className}`}><i><DetailIcon label={label} /></i><div><strong><span>{label}</span>{onTogglePrint && <button type="button" className={`detail-print-toggle no-print ${printVisible ? "is-visible" : ""}`} onClick={(event) => { event.preventDefault(); onTogglePrint(); }}>{printVisible ? "印刷あり" : "印刷なし"}</button>}</strong>{type === "area" ? <FormattedNumberInput value={Number(value)} decimals={2} onChange={(next) => onChange(String(next))} ariaLabel={label} /> : options ? <EditableChoice value={value} onChange={onChange} options={options} label={label} /> : type === "textarea" ? <textarea rows={2} value={value} onChange={(event) => onChange(event.target.value)} /> : <input type={type} value={value} onChange={(event) => onChange(event.target.value)} />}{secondary && <small>{secondary}</small>}{type === "area" && <span className="area-print-value">{formatNumber(Number(value), 2)}{secondary}</span>}{type === "month" && <span className="detail-print-value">{formatMonthJa(String(value))}</span>}</div></label>;
}
...
function DepreciationChart({ age, life }: { age: number; life: number }) {
  ...
  useEffect(() => {
    ...
    const left = 54, right = width - 28, top = 20, bottom = height - 35;
    ...
    const ticks = [0];
    for (let year = 5; year <= safeLife - 5; year += 5) ticks.push(year);
    if (ticks[ticks.length - 1] !== safeLife) ticks.push(safeLife);
...
export default function Home() {
  ...
  const [propertyName, setPropertyName] = useState(""); const [address, setAddress] = useState(""); const [mansionName, setMansionName] = useState(""); const [detailAddressValue, setDetailAddressValue] = useState(""); const [detailMansionNameValue, setDetailMansionNameValue] = useState(""); const [appraisalDate, setAppraisalDate] = useState(localToday); const [staff, setStaff] = useState("");
  ...
  function moveComparable(index: number, direction: -1 | 1) { setComparables((current) => { const target = index + direction; if (target < 0 || target >= current.length) return current; const next = [...current]; [next[index], next[target]] = [next[target], next[index]]; return next; }); }
  function changeUnitRangeMode(next: boolean) {
    setUnitRangeMode(next);
    setUnitManual(false);
    setTargetUnitManual(false);
    if (!next) {
      const adjustment = roundOne((adjustLow + adjustHigh) / 2);
      setAdjustLow(adjustment);
      setAdjustHigh(adjustment);
    }
  }
  function shiftAdjustment(amount: number) {
    setAdjustLow((current) => roundOne(current + amount));
    setAdjustHigh((current) => roundOne(current + amount));
    setTargetUnitManual(false);
  }
  ...
  const detailEditors = type === "land" ? [
    <DetailEditor key="address" icon="⌖" label="所在地" type="textarea" value={detailAddressValue} onChange={setDetailAddressValue} />,
    ...
    <DetailEditor key="road" icon="▤" label="接道状況" value={road} onChange={setRoad} printVisible={showRoadInPrint} onTogglePrint={() => setShowRoadInPrint((current) => !current)} />,
    ...
    <DetailEditor key="address" icon="⌖" label="所在地" type="textarea" value={detailAddressValue} onChange={setDetailAddressValue} />,
    <DetailEditor key="mansion" icon="⌂" label="マンション名" type="textarea" value={detailMansionNameValue} onChange={setDetailMansionNameValue} />,
    ...
    <div className="report-stack">
      ...
      <article id="page-3" className="report-page comps-page">
        ...
        <section className="comparison-table-wrap"><table className="comparison-table"><colgroup><col style={{ width: "11.5%" }} />{comparables.map((comp) => <col key={`case-column-${comp.id}`} style={{ width: `${88.5 / comparables.length}%` }} />)}</colgroup><thead><tr><th>事例</th>{comparables.map((comp, index) => <th key={comp.id}><span>事例 {index + 1}</span><span className="case-move-actions no-print"><button type="button" onClick={() => moveComparable(index, -1)} disabled={index === 0}>◀</button><button type="button" onClick={() => moveComparable(index, 1)} disabled={index === comparables.length - 1}>▶</button></span><span className="case-header-actions no-print"><button className="import-button" onClick={() => setActiveImport(comp.id)}>レインズ取込</button><button className="clear-case-button" onClick={() => clearComparable(comp.id)}>クリア</button></span></th>)}</tr></thead><tbody>
          <tr><th>所在地</th>{comparables.map((comp) => <td key={comp.id}><input className="comparison-edit-only" style={{ fontSize: `${Math.max(6.5, Math.min(18, (182 * comparableTextScale) / Math.max(1, comp.address.length)))}px` }} value={comp.address} onChange={(e) => updateComp(comp.id, "address", e.target.value)} placeholder="ー" /><span className="comparison-print-only comparison-address-print" style={{ fontSize: `${Math.max(6.5, Math.min(18, (182 * comparableTextScale) / Math.max(1, comp.address.length)))}px` }}>{comp.address || "ー"}</span></td>)}</tr>
      ...
      <article id="page-4" className="report-page analysis-page">
        ...
        <aside className={`calculation-flow ${unitRangeMode ? "range-flow" : "single-flow"}`}><h3>査定価格の算出フロー</h3><div className="flow-box"><span>周辺成約坪単価</span><div className="unit-edit-row">{unitRangeMode ? <><FormattedNumberInput value={surroundLow} decimals={1} showZero onChange={(value) => { setUnitManual(true); setSurroundLow(value); }} ariaLabel="周辺成約坪単価 下限" /><b>〜</b><FormattedNumberInput value={surroundHigh} decimals={1} showZero onChange={(value) => { setUnitManual(true); setSurroundHigh(value); }} ariaLabel="周辺成約坪単価 上限" /></> : <FormattedNumberInput className="single-unit-input" value={surroundLow} decimals={1} showZero onChange={(value) => { setUnitManual(true); setSurroundLow(value); setSurroundHigh(value); }} ariaLabel="周辺成約坪単価" />}<small>万円／坪</small></div><button className="inline-auto no-print" onClick={() => setUnitManual(false)}>{unitRangeMode ? "平均±5％へ戻す" : "平均値へ戻す"}</button></div><b className="flow-symbol">×</b><div className="flow-box editable-flow"><span>対象物件の評価補正</span><div>{unitRangeMode ? <><FormattedNumberInput value={adjustLow} decimals={1} showPlus showZero onChange={setAdjustLow} ariaLabel="評価補正 下限" /><b>〜</b><FormattedNumberInput value={adjustHigh} decimals={1} showPlus showZero onChange={setAdjustHigh} ariaLabel="評価補正 上限" /></> : <FormattedNumberInput className="single-unit-input" value={adjustLow} decimals={1} showPlus showZero onChange={(value) => { setAdjustLow(value); setAdjustHigh(value); }} ariaLabel="評価補正" />}<small>万円／坪</small></div><span className="adjust-stepper no-print"><button type="button" onClick={() => shiftAdjustment(.1)}>▲</button><button type="button" onClick={() => shiftAdjustment(-.1)}>▼</button></span></div><b className="flow-symbol equals-symbol">＝</b><div className="flow-box gold-box"><span>対象物件の査定坪単価</span><div className="target-unit-edit">{unitRangeMode ? <><FormattedNumberInput value={adjustedLow} decimals={1} showZero onChange={(value) => { setTargetUnitManual(true); setTargetUnitLowManual(value); setTargetUnitHighManual(targetUnitManual ? targetUnitHighManual : adjustedHigh); }} ariaLabel="対象物件の査定坪単価 下限" /><b>〜</b><FormattedNumberInput value={adjustedHigh} decimals={1} showZero onChange={(value) => { setTargetUnitManual(true); setTargetUnitLowManual(targetUnitManual ? targetUnitLowManual : adjustedLow); setTargetUnitHighManual(value); }} ariaLabel="対象物件の査定坪単価 上限" /></> : <FormattedNumberInput className="single-unit-input" value={adjustedLow} decimals={1} showZero onChange={(value) => { setTargetUnitManual(true); setTargetUnitLowManual(value); setTargetUnitHighManual(value); }} ariaLabel="対象物件の査定坪単価" />}<small>万円／坪</small></div><button className="inline-auto no-print" onClick={() => setTargetUnitManual(false)}>自動値に戻す</button></div><div className="down-arrow">▼</div><div className="flow-box final-flow"><span>{type === "mansion" ? "マンションの査定価格" : "土地の査定価格"}</span><strong><PriceValue value={landAppraisalLow} />{unitRangeMode && <>〜<PriceValue value={landAppraisalHigh} /></>}</strong><small>※上記価格は、対象物件の条件・市場動向を総合的に判断した目安です。</small></div></aside>
