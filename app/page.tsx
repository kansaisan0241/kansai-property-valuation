...
function Field({ label, value, onChange, type = "text", suffix, className = "" }: { label?: string; value: string | number; onChange: (value: string) => void; type?: string; suffix?: string; className?: string }) {
  return <label className={`field ${className}`}>{label && <span>{label}</span>}<span className="field-control">{type === "textarea" ? <textarea rows={2} value={value} onChange={(event) => onChange(event.target.value)} /> : <input type={type} value={value} onChange={(event) => onChange(event.target.value)} />}{suffix && <small>{suffix}</small>}</span></label>;
}
...
export default function Home() {
  ...
  const detailEditors = type === "land" ? [
    <DetailEditor key="address" icon="⌖" label="所在地" type="textarea" value={address} onChange={setAddress} />,
    ...
    <DetailEditor key="transport" icon="◎" label="最寄り駅・交通" type="textarea" value={transport} onChange={setTransport} />,
  ] : type === "mansion" ? [
    <DetailEditor key="address" icon="⌖" label="所在地" type="textarea" value={address} onChange={setAddress} />,
  ...
  return <main>
    ...
    <div className="report-stack">
      ...
      <article id="page-3" className="report-page comps-page">
        ...
        <section className="comparison-table-wrap"><table className="comparison-table"><thead><tr><th>事例</th>{comparables.map((comp, index) => <th key={comp.id}><span>事例 {index + 1}</span><span className="case-header-actions no-print"><button className="import-button" onClick={() => setActiveImport(comp.id)}>レインズ取込</button><button className="clear-case-button" onClick={() => clearComparable(comp.id)}>クリア</button></span></th>)}</tr></thead><tbody>
          <tr><th>所在地</th>{comparables.map((comp) => <td key={comp.id}><input className={comp.address.length > 26 ? "address-very-long" : comp.address.length > 18 ? "address-long" : ""} value={comp.address} onChange={(e) => updateComp(comp.id, "address", e.target.value)} placeholder="所在地" /></td>)}</tr>
          ...
          <tr><th>建物面積</th>{comparables.map((comp) => <td key={comp.id}>{type === "house" ? <div className="area-inline"><FormattedNumberInput value={comp.buildingArea} decimals={2} onChange={(value) => updateComp(comp.id, "buildingArea", value)} ariaLabel="建物面積" /><small>㎡（約{formatNumber(comp.buildingArea / TSUBO, 2)}坪）</small></div> : <span className="empty-value">ー</span>}</td>)}</tr>
          ...
          <tr><th>間取り</th>{comparables.map((comp) => <td key={comp.id}>{type !== "land" ? <input value={comp.layout} onChange={(e) => updateComp(comp.id, "layout", e.target.value)} placeholder="4LDK" /> : <span className="empty-value">ー</span>}</td>)}</tr>
      ...
      <article id="page-strategy" className="report-page strategy-page">
        ...
        <section className="price-strategy"><h3>価格戦略のイメージ</h3><p>ご希望に合わせて、以下の3つの価格戦略をご提案いたします。</p><div className="strategy-cards"><div className="strategy-card"><h4>SPEED<small>スピード売却価格</small></h4><strong>{formatNumber(speed)}<small>万円</small></strong><p>市場に売却を完了したい方向けの<br />スピード重視戦略</p><ul><li>早期成約が期待できる</li><li>販売期間：短期での成約を目指す</li><li>早く現金化したい方向け</li></ul></div><div className="strategy-card recommended-card"><em>おすすめ</em><h4>RECOMMEND<small>推奨売出価格</small></h4><strong>{formatNumber(recommended)}<small>万円</small></strong><p>市場での競争力と成約までの<br />期間を考慮した価格</p><ul><li>成約の可能性が最も高い価格帯</li><li>適正な期間での成約が期待できる</li><li>価格とスピードのバランス重視</li></ul></div><div className="strategy-card"><h4>CHALLENGE<small>チャレンジ価格</small></h4><strong>{formatNumber(challenge)}<small>万円</small></strong><p>相場より高めの価格から<br />市場の反応を確認する戦略</p><ul><li>高値での成約を目指す</li><li>販売期間：やや長期化の可能性</li><li>できるだけ高く売りたい方向け</li></ul></div></div></section>
