...
export default function Home() {
  ...
  function shiftAdjustment(target: "low" | "high" | "both", amount: number) {
    if (target !== "high") setAdjustLow((current) => roundOne(current + amount));
    if (target !== "low") setAdjustHigh((current) => roundOne(current + amount));
    setTargetUnitManual(false);
  }
    ...
    <div className="report-stack">
      ...
      <article id="page-4" className="report-page analysis-page">
        ...
        <aside className={`calculation-flow ${unitRangeMode ? "range-flow" : "single-flow"}`}>
          <h3>査定価格の算出フロー</h3>
          <div className="flow-box"><span>周辺成約坪単価</span><div className="unit-edit-row">{unitRangeMode ? <><FormattedNumberInput value={surroundLow} decimals={1} showZero onChange={(value) => { setUnitManual(true); setSurroundLow(value); }} ariaLabel="周辺成約坪単価 下限" /><b>〜</b><FormattedNumberInput value={surroundHigh} decimals={1} showZero onChange={(value) => { setUnitManual(true); setSurroundHigh(value); }} ariaLabel="周辺成約坪単価 上限" /></> : <FormattedNumberInput className="single-unit-input" value={surroundLow} decimals={1} showZero onChange={(value) => { setUnitManual(true); setSurroundLow(value); setSurroundHigh(value); }} ariaLabel="周辺成約坪単価" />}<small>万円／坪</small></div><button className="flow-mode-toggle no-print" onClick={() => changeUnitRangeMode(!unitRangeMode)}>{unitRangeMode ? "幅あり" : "単価1つ"}</button></div>
          <b className="flow-symbol">×</b>
          <div className="flow-box editable-flow"><span>対象物件の評価補正</span><div>{unitRangeMode ? <><span className="step-input-wrap"><FormattedNumberInput value={adjustLow} decimals={1} showPlus showZero onChange={setAdjustLow} ariaLabel="評価補正 下限" /><span className="input-stepper no-print"><button type="button" onClick={() => shiftAdjustment("low", .1)}>▲</button><button type="button" onClick={() => shiftAdjustment("low", -.1)}>▼</button></span></span><b>〜</b><span className="step-input-wrap"><FormattedNumberInput value={adjustHigh} decimals={1} showPlus showZero onChange={setAdjustHigh} ariaLabel="評価補正 上限" /><span className="input-stepper no-print"><button type="button" onClick={() => shiftAdjustment("high", .1)}>▲</button><button type="button" onClick={() => shiftAdjustment("high", -.1)}>▼</button></span></span></> : <span className="step-input-wrap single-step-input"><FormattedNumberInput className="single-unit-input" value={adjustLow} decimals={1} showPlus showZero onChange={(value) => { setAdjustLow(value); setAdjustHigh(value); }} ariaLabel="評価補正" /><span className="input-stepper no-print"><button type="button" onClick={() => shiftAdjustment("both", .1)}>▲</button><button type="button" onClick={() => shiftAdjustment("both", -.1)}>▼</button></span></span>}<small>万円／坪</small></div></div>
